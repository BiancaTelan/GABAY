...
from ..routers.backup_utils import perform_database_backup
...
def create_personnel(
    ...
    if emp_id is not None:
        existing_emp = db.query(Staff).filter(Staff.employeeID == emp_id).first()
        if existing_emp:
    ...
    new_user = User(
        email=data.email, passwordHash=hashed_password,
        role=assigned_role, isActive=True, is_verified=True 
    )
    db.add(new_user)
    db.flush() 

    new_staff = Staff(
        userID=new_user.userID, employeeID=emp_id,
        firstname=data.firstname, middlename=data.middlename, surname=data.surname,
        position=data.position, gender=data.gender, contactNumber=data.contactNumber,
        workingDays=data.workingDays, workingHours=data.workingHours
    )
    db.add(new_staff)
    ...
    background_tasks.add_task(
        ...
        recipient_email=data.email, name=f"{data.firstname} {data.surname}",
        role=data.role, raw_password=raw_password
    )
...
def get_personnel_list(db: Session = Depends(get_db)):
    ...
    for d in doctors:
        ...
        if d.schedule:
            ...
            schedule_str = ", ".join(list(dict.fromkeys(short_days))) # Distinct days
        ...
        formatted_personnel.append({
            "raw_id": d.docID,
            "id": d.employeeID or "Unassigned",
            "docID": f"DOC-{d.docID:04d}",
            "role": "DOCTOR",
            "name": f"{d.firstname} {d.middlename + ' ' if d.middlename else ''}{d.surname}",
            "firstname": d.firstname,
            "middlename": d.middlename or "",
            "surname": d.surname,
            "licenseNumber": d.licenseNumber or "",
            "email": d.email or "",
            "phone": d.contactNumber or "",
...
def update_personnel_page(
    ...
    # === IF STAFF / ADMIN ===
        ...
        if emp_id is not None:
            existing_emp = db.query(Staff).filter(Staff.employeeID == emp_id, Staff.userID != person_id).first()
            if existing_emp:
        ...
        if data.email and data.email != user.email:
            existing_email = db.query(User).filter(User.email == data.email, User.userID != person_id).first()
            if existing_email:
...
def add_doctor(data: DoctorCreate, request: Request, db: Session = Depends(get_db), current_admin: User = Depends(get_current_user)):
    ...
    new_doc = Doctor(
        ...
        licenseNumber=data.licenseNumber, email=data.email, contactNumber=data.contactNumber,
        deptID=data.deptID, isAvailable=True
    )
    db.add(new_doc)
    db.flush()
    ...
    for block in data.schedules:
        try:
            ...
            for day in block.days:
                if day in day_map:
                    db.add(Schedule(docID=new_doc.docID, weekDay=day_map[day], startTime=start_time, endTime=end_time, maxPatients=block.slot))
        except Exception:
            pass

    db.add(SystemLogs(userID=current_admin.userID, actionType=actionTypeEnum.INSERT, tableAffected="doctorTable", details=f"Registered Dr. {data.firstname} {data.surname}", ipAddress=request.client.host))
    db.commit()
...
def update_doctor(doc_id: int, data: DoctorUpdate, request: Request, background_tasks: BackgroundTasks, db: Session = Depends(get_db), current_admin: User = Depends(get_current_user)):
    ...
    db.add(SystemLogs(userID=current_admin.userID, actionType=actionTypeEnum.UPDATE, tableAffected="doctorTable", details=f"Updated details for: {target_name}", ipAddress=request.client.host))
    db.commit()
...
def toggle_doctor_status(doc_id: int, data: UserStatusUpdate, request: Request, db: Session = Depends(get_db), current_admin: User = Depends(get_current_user)):
    ...
    db.add(SystemLogs(userID=current_admin.userID, actionType=actionTypeEnum.UPDATE if is_activating else actionTypeEnum.DELETE, tableAffected="doctorTable", details=f"{'Reactivated' if is_activating else 'Deactivated'} Dr. {doc.firstname} {doc.surname}", ipAddress=request.client.host))
    db.commit()
...
def update_department(
    ...
    notifier.broadcast_sync({
        ...
        "desc": f"Updated department: {data.department}",
        "action": "UPDATE",
        "timestamp": datetime.now().isoformat()
    })
...
def get_all_appointments(db: Session = Depends(get_db), current_admin: User = Depends(get_current_user)):
    appointments = db.query(Appointment).options(
        ...
        joinedload(Appointment.action_by_user).joinedload(User.staff_profile) # Safely eager load staff
    ...
    for a in appointments:
        ...
        if a.action_by_user:
            if a.action_by_user.staff_profile:
                action_name = f"{a.action_by_user.staff_profile.firstname} {a.action_by_user.staff_profile.surname} (Staff)"
            else:
                action_name = "System Administrator"
        ...
        formatted_appts.append({
            ...
            "status": status_name,
            "date": target_date.strftime("%B %d, %Y") if target_date else "TBD",
            "rawDate": target_date.isoformat() if target_date else None, 
            "time": time_str,
